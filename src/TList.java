...
public class TList extends VisualComponent {
  ...
  public void repaint() {
	...
	app.gr.setColor(0x777777);
	app.gr.setStrokeStyle(Graphics.DOTTED);
	int page_delimiter_y = this.posY + (int)((this.rows - (this.start % this.rows)) * app.FontHeight[app.fontInd]);
	app.gr.drawLine(0, page_delimiter_y, this.width-this.scroll_width-1, page_delimiter_y);
	app.gr.setStrokeStyle(Graphics.SOLID);
    drawScroll();
