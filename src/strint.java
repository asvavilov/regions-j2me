public class strint {
    ...
    private int strtoint(String strV) {
      ...
      for (int L=1,i=this.length()-1;i>=0;i--,L*=10) {
        res+=L*strint.cif.indexOf(strV.charAt(i));
      }
      return res;
    }
    ...
    protected void set(int intV) {
      ...
      if (strV.length()<strint.maxlength) {
        strV="0"+strV;
      }
      this.strVal=strV;
    }
    ...
    protected void add(String strV) {
      if (this.length()==strint.maxlength) this.clear();
      if (this.length()==strint.maxlength-1 & this.getInt()==0 & strV.equals("0")) return;
      this.strVal+=strV;
